...
**MVVM, one window, tab-scoped state.** `MainWindowViewModel` owns the tab collection, sidebar, bookmarks bar, and window-level settings. Each open tab is an independent `TabViewModel` with its own path, navigation history (`_back`/`_forward` stacks), sort/view state, search, clipboard cut/copy, and a `FileSystemWatcher`. The **Settings tab** (`chrome://settings`-style) is just a `TabViewModel` constructed with `isSettingsTab: true` — `IsSettingsTab`/`IsNormalTab` gate which half of `MainWindow.axaml`'s per-tab `DataTemplate` renders. There is no separate settings window.
